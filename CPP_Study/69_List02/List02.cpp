...
{
public:
	Node() : _next(nullptr), _prev(nullptr), _data(T())// data는 T의 기본타입을 이용해서 초기화, int를 넣어주면 0으로 밀리게 된다.
	{
...
public:
	...
	// ++it
	Iterator<T>& operator++() // 자기 자신과 똑같은거는 <T>를 안넣어도 무방함.
	{
		_node = _node->_next;
		return *this;
	}
