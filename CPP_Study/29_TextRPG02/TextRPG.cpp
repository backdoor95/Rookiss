...
void CreatePlayer(StatInfo* playerInfo)
{
	...
	while (ready == false)
	{
		...
		{
		case PT_Knight: 
			playerInfo->hp = 100;// (*playerInfo).hp = 100 이거와 같은 의미
			playerInfo->attack = 10;// (*playerInfo).hp = 100 이거와 같은 의미
			playerInfo->defence = 5;// (*playerInfo).hp = 100 이거와 같은 의미
			ready = true;
			break;// 여기 break는 while이 아니라 switch를 빠져나가는 것이다.
		case PT_Archer:
			playerInfo->hp = 80;// 
...
void EnterGame(StatInfo* playerInfo)
{
	...
	while (true)
	{
		...
		// 매개변수 확인 단축키 = ctrl shift space
		CreateMonsters(monsterInfo, MONSTER_COUNT);// 배열의 크기를 넘겨줘야함
		// 배열은 함수에 그냥 넘어가는게 아니라. 주소형태로 넘어간다. 그래서 크기도 넘겨줘야한다.
		// 배열 자체를 넘길수 있는 방법은 없다.
